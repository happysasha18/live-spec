...
|---|---|---|---|
| full suite wall-time | ≤ 470 s on the dev machine [default] (measured 456 s serial at 2,404 tests with the suite-in-suite meta-test firing, 2026-08-06 on an idle machine; the meta-test alone takes 282 s of that, so a diff outside the gate-machinery class skips it and runs near 174 s. The row moves with the fresh measured figure each time the suite grows: it read 383–405 s at 1,856 tests on 2026-07-24. Queue row 553 holds the work that brings this number back down by narrowing the meta-test's own run) | the pytest tail line in the suite run's log, read by `guardrails/check-suite-budget.sh` on every full gate run and red past the budget naming both figures (M-346, row 361) | `guardrails/check-suite-budget.sh` reds past the budget on every full gate run, naming both figures (M-346) |
