...
#
# `tracked` is the raw bytes of a document this repository owns, matched exactly as written,
# so a needle only counts when it stands whole on the page. The re-pinning packet had swapped
# these reads for the flat one, which lets a phrase match across any line break and any
# indentation and silently widens every assertion here (fresh review finding 2).
#
# `canon` is the flat whole-surface read, used for exactly one home: the externalized
# product-prover canon, another repository's file whose wrapping this pack does not own and
# must not red on. The boundary is ownership, not convenience.
tracked = read
canon = read_all_flat
...
def test_the_tracked_reader_stays_locality_sensitive():
    ...
    newline. Asserting the tracked reader's output does contain one pins the raw read
    mechanically, rather than trusting the name at the top of the file.
    """
...
def test_severity_axis_retired_from_spec_and_readme_case_insensitive():
    ...
    for rel in ("PRODUCT_SPEC.md", "README.md"):
        hits = [m.group(0) for m in pat.finditer(tracked(rel))]
...
def test_architecture_owns_140():
    assert "INV-140" in tracked("ARCHITECTURE.md")
