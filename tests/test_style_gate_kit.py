...
class TestStyleGatesInstall(unittest.TestCase):
    ...
    def test_a_prior_install_s_leftover_lock_test_and_seeded_cap_are_removed(self):
        """Defect found in review (docs/prover/2026-09-02-q805-and-followups-review.md, finding 2):
        repairing the pre-push block stopped CALLING the retired lock test, but left the generated
        test file and its seeded cap sitting in the host's own tree — still collected by the host's
        pytest, still enforcing a ceiling this kit no longer seeds. A re-run must remove both, and
        must not touch an unrelated key sitting beside the seeded one in the same cap file."""
        with tempfile.TemporaryDirectory() as tmp:
            self._init_host(tmp)
            self._write_doc(tmp)
            os.makedirs(os.path.join(tmp, "tests"))
            os.makedirs(os.path.join(tmp, "scripts"))
            with open(os.path.join(tmp, "tests", "test_ratchet_lock.py"), "w", encoding="utf-8") as f:
                f.write("# generated by the retired ratchet kit\n")
            json.dump({"max_redundancy_open": {"DOC.md": 5}, "unrelated_key": "keep-me"},
                      open(os.path.join(tmp, "scripts", "spec-debt-cap.json"), "w"))

            result = run(["bash", INSTALL_STYLE_GATES, "DOC.md"], cwd=tmp)
            self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
            self.assertFalse(
                os.path.exists(os.path.join(tmp, "tests", "test_ratchet_lock.py")),
                "the retired kit's own generated test must be removed, not just un-called")
            cap = json.load(
                open(os.path.join(tmp, "scripts", "spec-debt-cap.json"), encoding="utf-8"))
            self.assertNotIn("max_redundancy_open", cap)
            self.assertEqual(cap.get("unrelated_key"), "keep-me",
                              "a field unrelated to the retired ceiling must survive untouched")

    def test_no_leftover_files_is_a_silent_no_op(self):
        """A host that never ran the old ratchet kit has neither file — the cleanup step must not
        fail or fabricate either one."""
        with tempfile.TemporaryDirectory() as tmp:
            self._init_host(tmp)
            self._write_doc(tmp)
            result = run(["bash", INSTALL_STYLE_GATES, "DOC.md"], cwd=tmp)
