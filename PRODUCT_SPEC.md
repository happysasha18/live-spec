# live-spec — Product Spec (v5.0.0, 2026-07-23)
...
6. The system *shall* run the prover-record, ownership, coverage, loadability, and prototype-fence checks at every push that carries content, never scoped, except: where criterion 7's deletion-only stand-down applies; where the inbox-deposit carve-out M-6 already carries stands the prover-record check down for a push whose diff is exactly one new file under `inbox/`; where the owner's recordless class — a range whose every commit touches only the record directories `docs/prover/`, `docs/skill-review/`, `docs/language-reads/` — stands the prover-record check down. [INV-40]
...
| INV-123 | R130.6 |
| INV-124 | R46.2, R67.4, R161.1, R161.2, R161.3, R161.4, R161.5, R161.6, R260.2, R300.18 |
...
| INV-145 | R68.1, R131.1, R131.2, R131.3, R191.9, R214.3, R215.2, R220.9 |
