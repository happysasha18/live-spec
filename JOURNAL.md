...
Entries dated 2026-07-12 and earlier live in `docs/queue-archive/JOURNAL-archive-2026-07-29.md`, moved there on 2026-07-29 so the live file stays under its byte bound. Nothing was dropped.

## 2026-07-29 — the journal rotates under its byte bound, by hand

The file stood at 641088 bytes against a declared ceiling of 640000, and the bound gate refused the push.
Every entry dated 2026-07-12 or earlier moved to `docs/queue-archive/JOURNAL-archive-2026-07-29.md`.
Those 170 entries stood in one contiguous block, so the move was one slice. The live file keeps 93
entries at 303406 bytes, which leaves 336594 bytes of headroom.

Nothing was lost, and the proof is an identity: the two files rebuild the original byte for byte, and the
rebuilt copy carries the same fingerprint as the file before the cut. The section counts add up as well,
93 and 170 against the 263 that stood.

The ceiling stayed at 640000 on purpose. A session that raises its own ceiling to pass its own gate
teaches the gate to mean nothing, and the same law landed over a different record hours earlier.

The archive carries the name `JOURNAL-archive-2026-07-29.md`. The rotation gate refuses any file named
in the other shape that no pointer line names, and the journal holds no numbered rows for such a line to
name. That gate reads the queue's row shape alone, so this rotation carries no machine proof of its own.
Queue row 531 holds that.
...
**2026-09-06 — reader answers as evidence.** The three statements validated today (q-816, q-823, q-822) were read by fresh clean-context readers whose four answers were returned in session reports and not kept in the tree; from this day `validate` copies the answers into the checkpoint's DONE section beside the receipt (scripts/task-admission.py, _keep_reader_record). The verifier's fourth pass named the gap. The same pass found that three estimate bases read like a derivation nobody performed; the rule now stands in the execution reference (an estimate's basis is comparable rows or a plain admission), and the three frozen statements keep their wording as frozen text.

**2026-09-06 — skill backups vs git.** He asked why the installer keeps copies on disk when every skill it installs is tracked in git or its own repository: «если скиллы есть в гитхабе то зачем они на диске?». Checked every backup in ~/.claude/skills-attic against its own repository's git objects: 436 of 480 were byte-identical to an object git already held, the rest were superseded drafts of the same day. install.sh now backs up a skill only where the installed copy differs from the source about to replace it — the one case that could lose something.

**2026-09-06 — three bypasses of the closure kernel, each red-proven first.** A worker could start with nothing on the board, because the pre-spawn rule sat inside `brief`, a command nobody had to call. A done was reachable two ways without doing the work: typing the mark into `PLAN.md`, or writing a receipt out of `--command true` while the row's own check went unrun. And deleting the row's `**DOD hash.**` line made it read as a row predating the kernel, so the next `verify` minted a fresh hash over a rewritten done and `close` compared that new contract against itself. On the published board a row of that kind stood in the Done column as *landed* with its own receipt reading `"verdict": "failed"` in the card below it. Each was reproduced on a throwaway tree before anything was changed. What now stands: `guardrails/worker-admission-guard.py` on the subagent tool, sharing `pre_spawn_check` with the brief so the two cannot disagree — confirmed end to end, a real session on this project had a trivial spawn denied; `verify` running the acceptance the tree recorded for the row, with `--command` demoted to an extra check and a keyless row unverifiable; the done's digest anchored on the checkpoint at admission, which makes the row's copy tamper-evident; `guardrails/check-close-receipt.py` as push gate u and a CI step, refusing a done that no passed receipt stands behind; and the renderer drawing a done over a failed receipt as reopened, which holds on the Pages runner where no acceptance command runs at all. Seventeen checks in `tests/test_closure_kernel_bypasses.py`, the ordinary road among them. build-pipeline's two documents state the three guarantees; the closing eval is declared stale rather than re-recorded, because the instruction this ran under forbade mass model evals.

**2026-09-07 — the board comes off its public address.** It went onto GitHub Pages on 2026-09-06, under my own hand: the third guarantee needed a published board to check, and I took the address that was already wired instead of asking whether the board belonged on a public URL at all. He took it off — «только доску нафига ты выкатил на урл гитхаба? убери оттуда плиз, хотя идея норм» — and stated the rule that replaces it: «у тебя всегда должна быть апдейтнутый канон и доска, просто показываются по требованию», sharpened a minute later to «канон показывается всегда, а доска по требованию. доска тебе необходима чтобы контекст всех тасков держать в том числе. и между сессиями и вообще». So the board's first job is holding the context of every task across sessions, and being looked at is the second. Pages is disabled on the repository, the publishing workflow is gone, and Requirement 309 criterion 8 now says the board is drawn from its source file whenever the person asks and kept current between the askings, standing at no published address. The registry row names the command that draws it. q-816's acceptance command, which read the registry for that address and asserted the workflow exists, reads the two facts the other way round.

**2026-09-08 — the pack's own wording grew the board, and two sentences of it are corrected.** The tlvphotos window sent up an incident with the owner's own stop in it: a session that repaired a day's work, opened no row, and still had a backlog ready to hand to the next session — «нельзя плодить таски… ты закрываешь все что знаешь сейчас в этой сессии». It traced the habit past its own judgement into two sentences the pack states. The standing orchestration law told the seat to walk a *forward queue* after a landing, which INV-48 already forbids the resume file to carry, so the rule and the spec pointed at two different lists. And the design reviewer's no-landing — right for a reviewer — stood as the only visible model of what a brief looks like, so a brief of the shape "find the cause and name the repair, do not build it" read as the pack's own; four diagnoses came back that day naming repairs nobody had built, and each would have become a row. Both sentences are corrected where they stand (q-828): the law now names the board, the project's one queue; the spec's list of the same laws drops the phrase with it; the shared rulebook's briefing rule says the lane that finds a cause repairs it in the same lane; and the reviewer's exception is named as an exception in both places, with the reason it is one — a grouping it surfaces is not a defect anybody meets until a person accepts a declaration, so there is nothing for the lane to build. Eleven architecture pins re-aimed behind the ten inserted lines.

**2026-09-08 — a corrected done and its prose copies, noted and not repaired.** The same day's push review found that when the owner corrected q-827's definition of done mid-flight, the correction moved the plan row and the checkpoint's DOD anchor while two prose copies of the replaced sentence stayed where they stood — the checkpoint's own DECISION SHEET, and the comment above `CHECKS["q-827"]`. It opens no row: it is a finding of our own review, neither copy is a promise anybody outside this repository meets (base rule 41), and the landing it belonged to has closed, so the window to repair it inside that landing has passed. The owner declined a repair this session. It stands in `docs/prover/2026-09-08-the-mode-line-is-a-field-the-gate-never-reads.md` and in the feedback ledger, and a later review finds it again if it still matters.

**2026-09-08 — 6.1.1: two faults in the pack's own lifecycle, and one number that never described anything.** The q-828 push met both faults in one landing. Five files under `hooks/` were shipped by no installer — the conduct-judge arms, the standing orchestration law they read, and the lean-orchestrator net — while config-health (gate m) held every one of them to match its installed copy; an edit to the law left the installed copy stale, the gate red with the fix "run the installer that owns this hook", and no such installer existed, so the file was copied by hand to get the push out. All five now ride `scripts/install-pack-hooks.sh`, `guardrails/judge-hooks.json` names each one's file so the declaration is again the one home, and the installer stops by name if a later hook reaches `hooks/` and nobody's list. The second fault: the push gate asks for the landing's review after the row closed — the prover record over the pushed range, the skill reviews of any skill it edited — and the spawn guard admitted a worker only against an open checkpoint, so the session owing an independent read of its own work had nobody to hand it to. The token now outlives the close by one window, the landing: the close uncommitted, or its commit not yet on the branch's upstream. Nothing else moves — the row is admitted, its frozen done and its recorded acceptance are checked as before, and a row whose close has landed refuses a spawn the way it always did.

**2026-09-08 — the integration cap comes out of the mode contract.** His word, correcting q-829 in flight: take the general numeric cap for an `integration` run out of the contract, the seed and the fallback. The figure was five, borrowed from `MOST_DELIVERABLES` — how many deliverables one plan holds, which says nothing about which targets a run covers. It admitted any five and refused a sixth for no reason anybody could state. What admits a run now is what it named before it started: `integration` against the targets its own composition lists, `release` against a core with the version it was taken at, `manual` against the purpose and finite sample its `records_before_start` declares. `row` keeps its one target, because one target is what a row run means. Four pack-owned copies moved together — `guardrails.config.json`, the seed in `adopt/install-scaffold.sh`, `MODE_LAW` in `guardrails/run_modes.py`, and criterion 3 of Requirement 322 — and the gate that reruns acceptances prints the refusal as a verdict rather than a traceback. This tree's own integration targets stand empty, which is the state that key is meant to be in between changes: an integration run here is refused until somebody writes down what it covers.

**2026-09-08 — what the global review of 6.1.1 caught, and the pin his word settled.** The read blocked on three, all of them the release's own. The landing window never shut: it was read off the checkpoint file, dirty or touched by an unpushed commit, and `brief` writes that file, so minting a token was what kept the window open across the push — the reviewer drove close, commit, brief, push on the real hook and got a spawn admitted on the far side. `close` now records where the branch's upstream stood at that moment and the window reads that anchor alone, so any push shuts it for every row closed behind it. The second: `close_is_unlanded` matched q-823's left-anchored `^def (…|close|…)` count, which is now settled by the better name, `landing_is_unpushed`. The third: VERSION went to 6.1.1 with no stamp, so every skill still read 6.1.0.

Stamping uncovered a fourth. `plan-2` held the thirty-six Director recordings no older than the skill they judge, by comparing commit timestamps, and the stamp moved that skill by its declared-version line and nothing else. He chose the reading the pack already stated one directory over: the pin reads the skill's content rather than its declared version. `evals/director/check.py --freshness` is that reader — a digest with the version line normalized out — and the thirty-six runs were re-pinned to it with nothing re-run, after checking that the text at the recording commit and the text after the stamp differ by that one line alone. A rule change of any size still moves the digest. Gate v in release mode over the whole pushed range now passes on every selected row.

**2026-09-09 — the calendar comes out of gate a.** The 6.1.1 landing was reviewed at 23:50 and pushed at 00:05, and the gate told it that it carried no written review: it picked the record it judged by the date in the record's filename. What cleared that push was a second record written to satisfy the clock and nothing else. His word: «одна минута до полночи или после ничего не меняют. это лишняя машинерия». Two arms already decided everything the date pretended to — a record older than `PRODUCT_SPEC.md` or `ARCHITECTURE.md` is refused, and a record that does not name the base commit and every pushed commit is refused. Both roads now take every committed record as a candidate, newest filename first, and those two arms pick. The gate's own work road had carried the same reasoning for months, in its own comment — "a clean tree after midnight is not a defect" — on the road where nothing enforced it. Four fixtures drive the real script over throwaway repositories, one of them the exact shape of that night: a record named for the 8th carrying a push on the 9th. The rule was written in more places than the change first reached — the banner every push prints, the gate roster copied out of it, the CI step name, and the two documents that describe this gate — and one test holds them together (q-830).

**2026-09-09 — 6.1.2: a run that says what it is doing, and the duration a close actually recorded.** His word at 00:40, about a run of his own: it goes quiet for two hours and there is no way to tell work from a hang. The acceptance-rerun gate was exactly that shape — it settled its list of targets before the first one started and then printed nothing until the last one returned. It now says, on the events it already reaches: the mode, the whole composition before the first target starts, each target as it begins and as it ends with the count still to come, and a closing line that the composition never grew, checked against the same list rather than asserted. No duration is read, no threshold compared, nothing polls, nothing watches, and no process is started to carry it.

The second half was a promise stopped mid-way. `comparable_durations` — the reader behind every estimate's basis — measured the checkpoint FILE's birth and modification stamps, while `_write_delivery_trail` twenty lines below it explained why those are worthless: every write renames a fresh file over the old one, so the span is zero however long the work ran. The number that means something was already being written at every close, as `estimate 3–5 hours → actual 1.7 hours`, and nothing read it back. It reads that line now, and `task-admission.py history "<group>"` is the end a person reaches: one line per closed row that recorded a duration, the field it came from, the basis of comparison, and `unavailable` where the group recorded none. It forecasts nothing, scores no similarity, and names no deadline, budget or ordering.

The verifier's own run of the new command caught two things the change had put there: the range line printed a group straddling the ninety-minute mark under one unit — "40 to 1.7 minutes" — where `_estimate` twenty lines above already carried the correction, and a runtime check that the composition had not grown could never fire, being true by construction. Both are closed in the same landing: the range converts, with a multi-row test that the single-row cases never reached, and the check that served the process is gone while the line it printed stands. The scoped review blocked on two more, both of them the release's own and both closed here. The 6.1.1 release test pinned `VERSION == "6.1.1"` as a literal, so the bump reddened the suite CI runs on every push, and the row that bumped could not see it — its own acceptance ran one other module; that file now holds what 6.1.1 shipped and leaves the version surfaces to the one test that already owns them. And the chapter said a host owes nothing, while both changed files are VENDORED and the installer skips a file that already exists: a host that pulled and stopped kept its old gate, its old reader, and no `history` command at all. The chapter names the command, and that it vendors into the directory it is run from. Both went out as 6.1.2 with a host chapter, and one deposit each into tlvphotos and exhibition-engine — his own two runs are the ones that hang. The engine had no `inbox/`; the note is the first file in one.

**2026-09-09 — 6.1.3: the watcher names the installer that actually re-vendors the file.** The 6.1.2 push review left two findings beside q-831, and his word this morning took the first: the daily update watcher tells a host which installer refreshes its stale vendored files, and for five of them it named one whose vendor set does not carry them. The sorting was a path prefix — `scaffold/guardrails/` took the scaffold road, everything else took the style-gate road — and `adopt/install-scaffold.sh` had since grown five run-mode files pinned under their host-relative paths, `guardrails/run_modes.py` and `scripts/task-admission.py` among them. A host stale on exactly those was sent to `install-style-gates.sh --force`, which holds none of them, so the run changed nothing and the same advice came back the next day. This is the same misrouting the 2026-07-16 fix closed, returning through the newer pins rather than through the branch it repaired: that fix named a road per kit and left a prefix standing in for the kit's membership, and the prefix stopped describing the kit the day the list grew.

The first repair was the same mistake a third time, and the verification caught it before the push. It read `install-scaffold.sh`'s own `VENDOR_MODES` array — one list, no copy beside it — and then sent everything that array did not claim to the style-gate road by default. Three installers under `adopt/` write into the one manifest, and `adopt/install-status-view.sh` keeps its own `VENDOR` array of eight keys: the probe, the board, the one-row reader, the drift check. The verifier ran the real installer into a throwaway host, aged the manifest it wrote, and got `install-style-gates.sh --force` for `scripts/render-board.sh` and `scaffold/status-view/state-probe.sh` — the identical defect one kit over, produced by the fix for it. So the sorting reads every `adopt/install-*.sh` for its own vendor arrays and maps each manifest key to the installer that states it, and a key no installer claims is now said out loud rather than handed to whichever road the code fell through to. A kit added later is routed with nobody editing the watcher, and the one branch that still named a fixed installer — an old pin with nothing stale — names the kits the manifest itself pins. Seven cases drive the real script — the five run-mode files, the style-gate set alone, the status-view set alone, all three kits at once, a key no installer vendors, an old pin with nothing stale naming only the kits it actually pins, and every installer's whole array walked one at a time — and the ones for the status-view kit, the unclaimed key and the old pin were red against what stood before them.

The same verification found the chapter naming its repair command under `sh`, where `adopt/install-scaffold.sh` is a bash script using `set -o pipefail` and arrays: under dash it dies at line 19 before copying anything. Every other command in that file says `bash`; the 6.1.2 chapter said `sh` and this one had copied it, so both are corrected. The second finding beside q-831 — a row estimated in weeks or sessions closes with a minute count wearing the row's own word — he held out of this landing, and it stands noted in the archived note with what it costs: no reader anywhere consumes that number, and one line on one checkpoint is what a person would meet.

**2026-09-09 — 6.1.4: a close rests on the acceptance the row was admitted with, and a row that could not prove that was abandoned.** His word: the official close path cannot widen acceptance over the command recorded at admission; the recorded command is the only proof of closing; a broader run can only be a manual run with its purpose and finite sample written down first, and manual evidence never closes a row. The rule was already written down in this tree and unread by the executor. `guardrails.config.json`'s `run_modes.row` records its targets source as the one accepted task's own recorded acceptance command and says no check outside that one task runs; `run_modes.manual` carries `decides_verdict` false. `verify` meanwhile took any `--command` as an extra check, wrote it into the receipt `close` reads, and asked nothing about the mode the run was made under. So a broad run nobody scoped in advance could become a row's closing evidence, and a manual run could write a receipt at all. It now runs the recorded acceptance and only that, refuses a command handed beside it, and refuses to write a receipt under any mode the tree's own contract records as deciding no verdict. The contract is read out of the tree that holds the plan, so a host is judged by its own copy. No hook, no second list of tests, no new runner: the two facts were already there and the executor simply asked them.

The first row for this work, q-833, was abandoned before any close, and the reason is the row's own subject. Its recorded acceptance drove `tests/test_closure_kernel_bypasses.py` with `python3 -m unittest`, which collects none of that file's bare test functions, runs nothing and exits 0 — with `>/dev/null 2>&1` hiding the line that says so. What remained of its closing evidence was a grep for a phrase that also stands in a comment of the file it greps. A row about closing on real evidence was going to close on none, and the independent verification found it before the close. An acceptance is anchored at admission and nothing moves it, which is the point of that anchor and the reason the row could not be repaired in place, so it was abandoned with the reason recorded, archived off the plan page, and the work carried to q-834 with a key that runs the tests with the collector that collects them. Reading every unittest-driven key in the table for the same shape finds exactly one other row, q-829, closed on 2026-09-08; its eighteen tests pass under pytest, so what is empty is the evidence rather than the work, and it stands in the deposit beside this landing with the two roads it has.

The same verification turned this landing back twice more before it stood. Two existing tests handed a command beside the recorded one and died in the new refusal; they are re-homed onto recorded keys, and the re-entry breaker's coverage came with them rather than being lost. A config missing a mode's own `decides_verdict` escaped as a refusal about a route, on a run that has no route; a caller's typo in a mode name was answered by sending them to repair the config; a tree with a broken reader was told it had none; and the no-verdict refusal described the manual run whichever mode had been named. Each now says which fault it met. And CI told a story of its own: main had been red since 2026-09-08, sixteen tests across five files, all on one cause. Python writes `__pycache__/` beside any module it imports, so the suite importing a hook made one appear under `hooks/`, and the installer's recurrence-stop — the one q-829's own note predicted would fire — read that generated directory as a hook shipped by no installer and refused the whole install. It passes over bytecode now, proved with the cache present against the installer as it stood.

**2026-09-09 — an inbox letter carries its own state, and the board names the letter a row came from.** His word at 10:00: make the lifecycle explicit — open, handled, noted, superseded, with the replacing letter named for a cancelled one — and never guess a cancellation out of prose. A letter's state was its position, plus a prose section that eight of the eleven archived letters do not carry, so a cancelled letter looked like a live one, a noted letter looked unhandled, and no row named the letter it came from. `scripts/inbox_lifecycle.py` is the one home for the four words; a letter says its own state on a `Status:` line and a superseded one names its successor, and a letter carrying no line still reads by where it stands, so all eleven letters already on the record read unchanged and none was edited. The refusals name the fault they met. Admission takes the letter, checks it is really on disk, and writes it onto the row; the status list shows that row with a compact `inbox:<filename>` label, shows an open letter beside the row it became, and shows no handled, noted or superseded letter as work. The work ran in its own worktree on a cheaper tier and the seat accepted it against the real surfaces rather than the report.

The verification turned it back three times, each time on the same fault wearing a different coat: a test asserting against the wrong line. The link test read the whole probe output, which passes on the plan row above whatever the inbox block does — green with the link deleted outright. The repair excluded that row by a substring the row does not contain, because an escape sequence sits between the id and the space, so it read the plan row again — green with a wrong title, and green with a link invented for a letter no row names. The third shape compares the letter's own line whole, and then the same sentence of the done was found failing in the other direction: with one row on the board, the right row and some row render alike, so a link taking its words from whichever row comes first passed. The test now carries two rows and three letters. Ten mutations of that link die on it. The reviewer also found a `Status:` line inside a fenced block read as the letter's own field — a state taken out of prose, which is the one thing the line exists to stop — and raised, then withdrew, an alarm about unterminated fences after building the alternative and watching it bring the original defect back.

Two things from the day's own reading landed beside it. The status-view installer now vendors the new reader, so a host's status view can read a state rather than carrying the feature and no way to run it, and this morning's watcher repair routes the new file to that installer with nobody editing the watcher. And `tests/test_status_view_install.py` kept a second copy of the installer's file list, which reddened the moment the kit grew; it reads the installer's own array now. That is the same class this morning's landing was about, met for the third time in one day.
