...
is a request for a change the product does not yet carry. A wish lands when the delivery that
completes it ships. Intake is continuous, a wish entering the queue the moment it is spoken.
Execution runs at most the lane cap of independent landings at once, three by default, so up to
three rows may read *in-work*. A landing finishes before a colliding next one starts, and two
landings collide when they rewrite one clause or one behaviour's rule.

The roadmap is a member of the format family. Its shared rules live once in `docs/spec-format.md`
...
line names the rows that moved and the file that received them. A script maintains this block, and
a gate cross-checks it against the archive.
