...
### ✅ Admission reads the record before it writes a row — id: q-825
**Group:** Pack quality · **Priority:** normal
...
**Read before admission.** q-823 (PLAN.md), q-822 (PLAN.md), 2026-09-04 (DECISIONS.md), 186dc4a2 (commit 186dc4a2), q-824 (PLAN.md). Finding: No row on the record covers reading the record at the door. q-823 and q-822 moved the closure kernel and split the Director, and neither touched what admission reads before it writes. The decision record holds nothing on repeating closed work, and 186dc4a2 is the range this lands on. The nearest standing law is Requirement 43's three-source impact read, which reads the spec, the architecture and the code — the product's own sources, and not the project's record of what it already settled. q-824 is this same work, abandoned the same evening: it was admitted against an acceptance command that runs pytest, which the session-start probe forbids, and the acceptance is anchored at admission, so that row had no road out.

**Supersedes.** q-824 (PLAN.md). New here: the door now refuses an acceptance command that runs a test suite, so this dead end cannot be entered again (Requirement 321 criterion 6a, M-656), and this row carries a check the probe can run. Why the earlier decision does not cover it: q-824 was abandoned without landing; its acceptance was frozen at admission against a command the suite forbids, so the work it names has never been accepted by anything
