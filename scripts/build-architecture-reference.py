...
named before the `-o` flag is read as ONE document, in the order given, and the table is built over
that whole concatenation — the sibling shape `build-matrix-reference.py` already carries. Naming the
core alone auto-expands to its full parts map (`specformat.spec_paths`'s own idempotent expansion), so
the pre-split single-argument call keeps working unchanged.

Usage:
  build-architecture-reference.py <ARCHITECTURE.md> [<part.md> ...]            # print the generated Reference table
  build-architecture-reference.py <ARCHITECTURE.md> [<part.md> ...] -o <file>  # write the generated table to <file>
Stdlib only.
"""
import os
import re
