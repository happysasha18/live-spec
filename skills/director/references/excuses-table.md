...
|---|---|
| "it's a one-liner / just a prototype" | The Room was "just to see" — it shipped hover-only with no phone layout and no spec. Size never picks the door; the tripwires do. |
...
| "explaining it would take longer than just doing it myself" | That is delegation dying, and the seat buried in junior work; write the self-contained brief (the junior-delegation rule in `SKILL.md`, expanded in [delegation-protocol.md](delegation-protocol.md)). |
