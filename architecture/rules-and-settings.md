...
- `skills/live-spec-base/SKILL.md:186` (rule 16, prototype fence)
- `skills/live-spec-base/SKILL.md:196` (rule 22, INV-98 — the convergence principle)
- `skills/live-spec-base/references/settings-ladder.md:1` (ladder — the on-demand module beside the rulebook; `skills/live-spec-base/SKILL.md:294` carries the pointer to it)
...
- `skills/live-spec-base/SKILL.md:222` (rule 26, INV-136/INV-139 — a project kind declares design principles the verify pass runs; the per-kind table lives in this doc)
- `skills/live-spec-base/SKILL.md:227` (rule 27, INV-143 — the seat decides what it can decide, surfaces only what it cannot)
- `skills/live-spec-base/SKILL.md:244` (rule 31, the earned-message law INV-183/INV-189 the named-reference machinery joins. The pair-travels register [E-35], the living-description heal [INV-240], and the earned auto-deposit [T-24] ride this rule's build, ROADMAP 424 [target]. The prover's station stands as their net until they ship, per [INV-150].)
- `skills/live-spec-base/SKILL.md:143` (rule 7's worker-restore sub-rule, INV-298 — the worker holds its own bytes, halts when it holds none, and the orchestrator owns recovery)
