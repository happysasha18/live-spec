...
**notes** —
- INV-198, INV-199, INV-201 (owned by parallel-lanes, the predicates' own home): this node runs three of their checks — `guardrails/check-config-health.sh`'s own new arm (pinned above), `guardrails/check-merge-base.sh` ahead of the landing gate, and `guardrails/check-worktree-line.sh` at the adoption gate — PLAN.md's q-804 row. INV-199's own stale-lane check (a `lane/*` branch or worktree with no open queue row) stays unbuilt, its own open row.
