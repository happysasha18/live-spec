...
- INV-324 (the success-measure feed's one reader, `scripts/check-success-measure-feed.py` — the automatic half of the success-measure reading machinery Requirement 76 promises, PLAN.md's q-48 row. The rule sits at `spec/success-measure-feed.md`'s Requirement 318; the machine that reads a feed and reds a skipped, empty, stale, or malformed one is this node's, the rule staying home with the requirement that states it and only the script's pin landing here — the same split the worker-restore and session-extract entries on the base-rulebook node already draw. The `[target]` tag on Requirement 76's own reading-machinery clause stays live: writing the fetch tooling that fills a feed from a real analytics account is each host's own job and is unbuilt in this tree. Printing a confirmed feed beside a project's rows belongs to the shared renderer, `scaffold/status-view/state-probe.sh`, which calls this script; both duties are stated at Requirement 318.)
