...
- INV-324 (the success-measure feed's one reader, `scripts/check-success-measure-feed.py` — the automatic half of the success-measure reading machinery Requirement 76 promises, PLAN.md's q-48 row. The rule sits at `spec/success-measure-feed.md`'s Requirement 318; the machine that reads a feed and reds a skipped, empty, stale, or malformed one is this node's, the rule staying home with the requirement that states it and only the script's pin landing here — the same split the worker-restore and session-extract entries on the base-rulebook node already draw. The `[target]` tag on the reading-machinery clause stays live and is owned by PLAN.md's q-824: this script reads a feed's own shape, and nothing yet binds a metric in that feed to the success-measure sentence of the feature it measures. Writing the fetch tooling that fills a feed from a real analytics account is each host's own job and is unbuilt here. Printing a confirmed feed beside a project's rows belongs to the shared renderer, `scaffold/status-view/state-probe.sh`, which calls this script; both duties are stated at Requirement 318.)
...
- INV-326 (what a priority means in a project's own words, and the next move derived from it: the statement lives in one place in a project's own plan — this pack's is PLAN.md's "Words used here" bullet — and `scripts/plan_checks_core.py`'s `read_priority_order` and `priority_rank` are its one machine reading. The renderer ranks inside a group by that order, prints an unnamed priority word last and visible, and derives the next move from the order rather than from a row's position on the page, printing beside it the word it won on. A plan stating no such list gets no invented order and is told so in the printed list. The rule sits at `spec/live-status-reporting.md`'s Requirement 320; PLAN.md's q-819 row.)
- INV-327 (the row-origin gate, `guardrails/check-row-origin.py`, gate ah: a finding of the project's own is repaired in the landing that found it or offered once, and every open row on the board carries one word naming what raised it — `asked`, `found` or `sent` — which the status view prints beside the row. `scripts/plan_checks_core.py` reads the word and names the three; the gate refuses an open row carrying none or carrying a fourth. The rule sits at rule 41 of this pack's rulebook and at `spec/live-status-reporting.md`'s Requirement 321; PLAN.md's q-825 row.)
...
- `scripts/plan_checks_core.py:135` (`read_priority_order`, the one reader of a plan's own priority statement, INV-326)
- `scripts/plan_checks_core.py:154` (`priority_rank`, which places a word the statement never names last, INV-326)
- `guardrails/check-row-origin.py:1` (the row-origin gate, gate ah, INV-327)
