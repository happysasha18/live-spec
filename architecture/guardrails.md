...
- INV-243
- INV-250..INV-263 (the requirements-format laws and their format-gate family, armed at the row-445 conversion delivery. The range once ran two codes further, to the size ratchet that closed the family; that gate and its recorded bound retired 2026-09-02, leaving the two numbers empty rather than renumbering the laws after them)
...
- INV-301 (the findings ratchet over every live document, held at zero for a cleared document. Its mechanical gate — gate aa, with its recorded-ceiling record, retired; the size ratchet it once had for a sibling retired the same way on 2026-09-02.)
...
**notes** —
- INV-198, INV-199, INV-201 (owned by parallel-lanes, the predicates' own home): this node holds three scripts PLAN.md's q-804 row built — `guardrails/check-config-health.sh`'s own new arm (pinned above), `guardrails/check-merge-base.sh`, and `guardrails/check-worktree-line.sh` — each real and individually tested. Only the first is actually called anywhere outside its own test (`guardrails/pre-push`'s gate m). A 2026-09-02 hostile review found the other two have no real caller — the landing walk names no merge-base call, the adoption/catch-up walk names no worktree-line call — so their own promises stay `[target]` until something in the real walk invokes them, not just their own fixture. INV-199's own stale-lane check (a `lane/*` branch or worktree with no open queue row) stays unbuilt too, its own open row.
