...
- INV-324 (the success-measure feed's one reader, `scripts/check-success-measure-feed.py` — the automatic half of the success-measure reading machinery Requirement 76 promises, PLAN.md's q-48 row. The rule sits at `spec/success-measure-feed.md`'s Requirement 318; the machine that reads a feed and reds a skipped, empty, stale, or malformed one is this node's, the rule staying home with the requirement that states it and only the script's pin landing here — the same split the worker-restore and session-extract entries on the base-rulebook node already draw. Writing the fetch tooling that fills a feed from a real analytics account is each host's own job and is unbuilt here. Printing a confirmed feed beside a project's rows belongs to the shared renderer, `scaffold/status-view/state-probe.sh`, which calls this script; both duties are stated at Requirement 318.)
- INV-325 (the status-view drift gate, `guardrails/check-status-view-drift.py`, gate ag: the pack ships one status renderer — `scaffold/status-view/state-probe.sh`, byte-identical to the pack's own `scripts/state-probe.sh` — and a project's own facts print through its own `scripts/state-probe-extras.sh` instead of naming the pack in the shared file. The rule sits at `spec/live-status-reporting.md`'s Requirement 319; PLAN.md's q-818 row. Two poles, one check: the repo carrying the shipped source is the pack and is compared against itself with no manifest needed; any other repo is read as a host, proving its vendored copies against the pack root its own manifest names, relative records resolved against the host and absolute ones taken as given. A comparison that opens no pairs, or a resolved root that turns out not to be the pack, never reads as a pass: each names the reason and stands down instead.)
