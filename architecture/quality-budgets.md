...
| work board update [target] | no stage delayed by its own board update [default] — the periodic ≤5 s auto-refresh heartbeat this budget once also carried retired 2026-09-03 on the owner's 2026-09-02 12:46 word (`.live-spec/turnkey-contract-composed.md:304`) | the page's own build stamp — the time the generator last wrote the page, read off the rendered board | the generator's own suite timing assertion at its landing (ROADMAP row 166); until that landing the budget stands unwatched [target] |
