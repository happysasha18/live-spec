...
- `skills/build-pipeline/SKILL.md:127` (the craft ladder — step→craft one home, Requirement 51 backs it at the SPEC level)
...
- `skills/live-spec-base/SKILL.md:145` (rule 7's lanes sub-rules — three lanes under one pen; the cap and the lane-open act have their one home here, and the director's reference points at it rather than restating it)
- `skills/live-spec-base/SKILL.md:177` (one row per landing commit)
