...
1. The system *shall* define the success-measure feed as one JSON file a host's own fetch tooling writes, carrying a generation timestamp, its source in plain words, and one or more named metrics, each carrying a label, a value, and a unit. [INV-324]
2. *when* a two-variant experiment is running, the system *shall* let the feed carry it as one named block holding exactly two variants, each with its own label and non-empty metrics list. [INV-324]
3. The system *shall* have `scripts/check-success-measure-feed.py` be the one reader of the feed's shape, run by any host wanting a feed checked before printing it. [INV-324]
...
6. *when* the feed's timestamp is older than the caller's staleness bound, the system *shall* red and name it stale. [INV-324]
...
9. The system *shall* leave writing the fetch tooling that produces a feed from a real source — a host's analytics account or traffic log — to each host, Requirement 267's own host pole. [INV-324, INV-163]
10. The system *shall* leave a host's status view printing a checked feed's numbers beside its tasks unasked to each host. [INV-324]
