...
9a. The system *shall* have `adopt/install-status-view.sh` record, under its own key in the host's manifest, the pack root it installed from — relative to the host root where the two trees resolve under one shared parent (the ordinary `~/live-spec` beside `~/my-project` layout), so a second clone, a second machine or CI reads its own pack rather than this machine's path; absolute only where that does not resolve, a convenience for the one machine it names. The system *shall* have the check resolve a relative recorded value against the host root, use an absolute one as it stands, and read that key when no `--pack-root` is given, so a host's own push gate needs no flag wired in to find the pack. `--pack-root`, when given, *shall* win over the recorded value. Where the resolved root does not itself carry the pack, the system *shall* stand the arm down rather than comparing against an unintended checkout. [INV-325]
