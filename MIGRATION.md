...
4. Re-run `bash <pack>/adopt/install-style-gates.sh --force`, the same re-vendor discipline as the two steps above but for the style-gate kit — the style lint, the near-duplicate reading, the freeze tool, the pre-show register lint, and their shared word lists. This is a founding-time step already (`adopt/ADOPT.md`, "Then wire the style gate"); catch-up owed it too and did not carry it before now — an already-adopted host running only steps 2 and 3 above never received a file this kit vendors (found 03.09, `inbox/2026-08-12-preshow-lint-script-missing.md`: `scripts/preshow-register-lint.py`, named as a required check by session law, shipped to no host tree because no installer's re-run road reached an already-adopted host).
5. Run the host's own gates, including the test suite where one exists. A red gate is the walk's own open defect: the walk stays open until the gates read green, and the checkpoint carries the red state across sessions.
6. Re-record the installed-set record in the current format.
7. Land one journal chapter in the host: what moved, why, the provenance, and any finding held for the owner.
8. Run the before-and-after self-test below and clear it before the walk is called done.
...
After the execute phase, record the same inventory again and compare the two. Every difference must be accounted for by a plan item — a file is unchanged, re-homed to a named new path, merged from named sources, or resting in the attic under its manifest line; an anchor-multiset delta must match a change the plan names; and the suite reads at least as green as before. One difference class is accounted for by name rather than by a plan item: a tracked file the host's own test runner rewrites on every run (a timings or cache artifact, `tests/suite_timings.json` the case found 2026-09-03) always differs between the pre-walk and post-walk fingerprint, on a host whose suite runs mid-walk at all — that is the runner's own known, non-leak side effect, not a walk-caused change, and naming the file class here is enough; it owes no separate plan item and never needs re-discovering per host. A difference outside the plan, and outside this named class, blocks the verify phase until the owner's gate accepts it as a plan amendment or the step is reverted.
