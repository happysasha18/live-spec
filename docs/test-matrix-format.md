...
1. **The artifact inventory** — a table of every file the reader receives, each entry owning at least one row that asserts it at the rendered level: the rung that inspects the real produced artifact, defined below under the pinned level. An artifact that no test renders can ship broken while the suite stays green, so the inventory closes that gap by construction.
...
- **the pinned level** — the rung the row's test sits at, drawn from the project's declared level ladder. `docs/test-method.md`'s level-ladder section names the ladder's four rungs: `string`, `DOM-text`, `browser-computed`, `pixel`. The level is the row's most important judgment, and the family's criterion form does not carry it, so the matrix row states it as its own field. The ladder's rungs are named by what proves them, so each project kind fills them with its own concrete proof artifacts. A project declares its own fill at founding, in the host profile's `project.proofs` line, and the row pins one of its own project's rungs. The rendered level named above is not a fifth rung. It is whichever of these four rungs a project's own fill inspects the real produced artifact at. For this pack, `TEST_MATRIX.md`'s own preamble names that rung as `string`, against the shipped file on disk.
