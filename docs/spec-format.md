...
- the criterion-readability ratchet — four arms over the acceptance criteria, one per reading defect the body survey measured: a criterion past the word cap, a term defined in place in a dash-pair aside or a parenthetical, a closing clause with no finite verb, and an anchor that competes with the prose. Each arm carries its own threshold and its own recorded count in `guardrails/criterion-readability.json`; a count above the recorded one is red, and a count below it re-baselines;
