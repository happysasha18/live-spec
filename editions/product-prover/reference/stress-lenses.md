...
- **Async pending, arrived, failed.** Find every piece of content the document produces
  asynchronously into a reserved on-screen slot. Assert that the document names its three states,
  pending, arrived, and failed, with a visible pending state wherever the slot holds a place. A slot
  that renders empty and silent while its content is in flight is the finding. The author writes each
...
- **The viewport quantifier.** Every layout guarantee names its viewport quantifier. It either
...
- **Same-kind-group handling.** Where the parts are a same-kind group no clause declared, a
  ...
  wide and short. Every consistency check read clean, because the claim was true as written.
...
check asserts the policy on every sibling surface the product renders.
...
exist. The principle may be a way in and out, a gesture, or a treatment. It is the same defect the
...
the parameters a person perceives across it. Where do focus and selection land? What scroll or
playback position holds? Does sound continue, does a timer keep running, and is a shown value fresh
or stale?
...
The motion-parity lens and the entry-state lens below are instances of this one. Each reads this lens
...
The platform default, a reset to the top, becomes the behaviour unreviewed. This lens names the
missing parameter, and the entry-state instance writes its sentence.
...
banner. A conditionally-entered face with no deliberate re-entry path is a finding. The document
...
is the question entry symmetry leaves unasked. It closes a class the two path lenses missed: a
gallery side panel reopened on the last picture a prior visit had scrolled its lane to, where no line
stated that the lane lands on the first member and resets at entry.
...
and a missing answer in any of them is a blank-answer finding. Motion feel is the decision-owner's
call, so an open motion question is surfaced to them in the batch of questions, marked as a
provisional default, and it holds nothing back.
...
- *The gesture's inverse (the reversibility of the means)* — a surface that opens by a continuous
  reversible gesture, such as a pinch, a drag, or a lift, owes that gesture reversed among its stated
  ways to close. A decided sentence for its absence answers the read as well.
- *The inverse's magnitude* — where the pair rides a continuous quantity, such as a pinch span, a
  drag distance, or a wheel accumulation, the document owes an answer on the inverse's size. The
...
- *Kind* — a declared one-sided pair is a `defect`, since a required answer is missing. A
  never-declared same-kind grouping over the same physical gap belongs to a design-consistency
  review's motion-parity check, which recommends.
...
read on reopen into an interface that no longer matches it? Is there a defined migrate, ignore, or
clear rule? This is the family of "reopened the widget and it looked broken", where persisted state
auto-restores into a changed surface.

**Scenario entry and exit** — a person-facing scenario is a flow, such as "walking the gallery",
"answering the quiz", or "when a bug cuts the line". For every one of them, check that the document
...
Walk every axis the surface passes through while it is already shown: view, mode, user tier,
viewport, reopen, and two writers acting on it at once. A relayout when the window changes shape
re-runs an entry animation nobody composed.
...
Report the missing seam. This review invents no answer and asks the decision-owner nothing. The
...
- **Surface authority** — when an operation creates, modifies, or removes an object of some category,
  ...
  three-source lens below supplies the missing evidence, since that document names the authoritative
...
- **Interactive overlap across layers** — one surface sometimes opens over another, as a modal, a
  zoom, or an overlay, and the covering surface carries its own controls. Read the document for every
  other interactive control that stays on screen while the overlay stands. Ask whether the document
  states that control is hidden or made unpressable.
  ...
  passive element may overlap freely: a caption, a plaque, the artwork. The rule binds the clickable
  controls.

  An ordinary test suite stays green while the running product collides, so a browser-level assertion
  is the render-time floor. This lens catches the blind spot earlier, reading the document's layered
  surfaces.
...
- **Approved-look clauses** — when a clause encodes an approved look, meaning a prototype somebody
  approved as the norm, read it twice. Does it point at the frozen copy of that approved artifact?
  ...
  text contradicting its own artifact. The pointer cites a frozen dated copy, and a pointer into a
...
- **Three-source disagreement** — read the change against the document, the architecture, and the
  ...
  question, which is what lets a reviewer derive an answer instead of raising a fork.
...
- **False serialization and over-broad independence** — when the document under review is a
  concurrency plan, such as a sprint board, a set of parallel work streams, or a task dependency
  graph, read every serialization it declares and every edge it draws. Two findings live here, one
  per side of the edge rule.
  ...
  only a document still open in parallel. The same finding covers two more shapes: an edge drawn
  where no movement needs another's finished output, and a same-section or same-behaviour collision,
  where the two rewrite one clause or one behaviour's rule.
...
- **Delivery separability along a declared axis** — the document under review sometimes declares a
  ...
  what the visitor receives divide along the axis, or ship as one piece? The composition question asks
  whether behaviour splits along the axis. Its dual reads whether the artifact the visitor receives
  ...
  architectural reason to ship whole, and no delivery road it owes. A stated reason reads as one
