...
**Twelve working [Claude Code](https://claude.com/claude-code) skills, plus the one shared rulebook they all load. They turn a wish spoken in passing into a specified, reviewed, tested, committed change. Scripts block the push when the documents and the code disagree.**
...
The rules are the part a software house would charge you for: twenty-two shared rules across the skill set, stated once in [`live-spec-base`](skills/live-spec-base/SKILL.md). They cover how a spec gets written so it stays readable. They cover when a question is worth your attention and when it is routine. They cover what a green suite does and does not prove.
...
- **Routine choices are made and reported; the work keeps moving.** Only what the documents genuinely leave open reaches you as a question.
...
That's the whole customer-facing story. Everything from here down is technical depth — exact formats, script internals, install internals, and the project's own fine print. None of it is required reading to use live-spec.
...
[`docs/adoption.md`](docs/adoption.md) explains the setup walk step 2 runs. The normative procedures it follows are [`adopt/ADOPT.md`](adopt/ADOPT.md) for an existing codebase and [`adopt/START.md`](adopt/START.md) for an empty directory.
...
That config then needs two things from you. First, fill in your paths: the spec, the test matrix, the queue, the tests, and the surface registry. The registry is the table listing every surface the project ships. A surface is one named part a user meets: a screen, a page, a command, a public function. The queue is the dated list of what has been asked of the product and where each ask stands. This repository keeps its own in [`PLAN.md`](PLAN.md).
...
The first vendors the style, redundancy, and freeze gates. It measures your documents as they stand today and writes the debt caps at those numbers. It generates `tests/test_ratchet_lock.py`, so the gate is green the day it lands and the debt can only hold or shrink from there. It ends by printing a fifth gate line for your push hook. That line belongs in the same `.git/hooks/pre-push` you just wrote, beside the four. Write it as `python3 -m pytest -q tests/test_ratchet_lock.py || exit 1`. The printed form ends `|| fail=1`, which fits a hook that collects every failure before it exits. That gate line labels it `gate r`. This repository's own hook set already gives that letter to a different check, so give yours another label if you carry that set.

The second puts the pack's own scan hooks on your machine, and this one reaches outside your project. It copies the hook files under `~/.claude/hooks/`. Two are wired into `~/.claude/settings.json` for you: the clock hook and the chat-law hook. The other four are opt-in from 2026-08-17; you wire one yourself, reading its command from `guardrails/judge-hooks.json`. Run it with `--dry-run` first: it prints every file it would copy, and touches nothing. Running it for real twice changes nothing, and your personal overlay files are never written by it. [`adopt/ADOPT.md`](adopt/ADOPT.md) counts both installers as part of the attach. The setup walk runs both of these for you too; they are here for a programmer setting a project up outside a live-spec session.
...
**Known issues.** Internal vocabulary still leaks into human-facing text. A register lint — [`scripts/preshow-register-lint.py`](scripts/preshow-register-lint.py) — blocks the leaks it already knows before an artifact is shown, and chat stays the weakest surface. The spec still carries style debt, counted and capped in [`scripts/spec-debt-cap.json`](scripts/spec-debt-cap.json), with the work to clear it dated in the queue, [`PLAN.md`](PLAN.md). The settings card is the page listing every setting the pack knows, its current value, and one plain line saying how to change it. That card is young and has run on one project. All three are tracked and reviewed at every push.

One more, structural rather than tracked: this repository's own `guardrails.config.json` still carries the scaffold's placeholder discovery pattern for an HTML section tag, and neither rendered artifact here is HTML — that pattern matches nothing in plain markdown, so the reverse completeness direction (a rendered surface with no registry row) is inert on this repo as a result. The forward direction is what the push actually enforces: every row in [`SURFACES.md`](SURFACES.md) matching real content. Not yet fixed — and this sentence itself is deliberately worded to avoid literally spelling out the pattern's own syntax, since the completeness check's substring scan has twice now caught its own description as a phantom surface (2026-08-18, and again tonight).
