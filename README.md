...
Three things have to be true before any of it works, and each has one command that says so. `python3 --version` must print 3.9 or newer; install a newer Python if it does not. `git rev-parse --is-inside-work-tree` must print `true` in your project's root; run `git init` there if it does not. And `git rev-parse --verify origin/main` must name a commit, because the tests-present check diffs your work against it; if that command fails — no remote, or a default branch named otherwise — you name your own base in the config instead, two paragraphs down. All of this assumes Claude Code on your machine: the pack is a set of skills it loads, and step 2 happens inside it.
...
The first vendors the style, redundancy, and freeze gates, measures your documents as they stand today, writes the debt caps at those numbers, and generates `tests/test_ratchet_lock.py`, so the gate is green the day it lands and the debt can only hold or shrink from there. It ends by printing a fifth gate line for your push hook, and that line belongs in the same `.git/hooks/pre-push` you just wrote, beside the four: write it as `python3 -m pytest -q tests/test_ratchet_lock.py || exit 1`, since the printed form ends `|| fail=1` for a hook that collects failures instead of stopping at the first. The printed recipe labels it `gate r`; if you have taken this repository's own hook set, that letter is already spoken for by a different check there, so give yours another label.

The second puts the pack's own scan hooks on your machine, and this one reaches outside your project: it copies the hook files under `~/.claude/hooks/` and writes their wiring into `~/.claude/settings.json`, which every project of yours reads. Run it with `--dry-run` first to see every file and every line it would add, and nothing is touched. Running it for real twice changes nothing, and your personal overlay files are never written by it. [`adopt/ADOPT.md`](adopt/ADOPT.md) counts both installers as part of the attach.
