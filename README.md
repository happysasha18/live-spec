...
That config then needs two things from you. First, fill in your paths: the spec, the test matrix, the queue, the tests, and the surface registry. The registry is the table listing every surface the project ships. A surface is one named part a user meets: a screen, a page, a command, a public function. The queue is the dated list of what has been asked of the product and where each ask stands. This repository keeps its own in [`ROADMAP.md`](ROADMAP.md).
