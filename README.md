...
The second puts the pack's own scan hooks on your machine, and this one reaches outside your project. It copies the hook files under `~/.claude/hooks/` and wires none of them. Every one of those six is opt-in from 2026-08-17. You turn one on by adding its command to `~/.claude/settings.json` yourself, reading its surface and its command form from `guardrails/judge-hooks.json`. The clock hook and the chat-law hook are the two the setup walk still wires for you. Run it with `--dry-run` first: it prints every file it would copy, and touches nothing. Running it for real twice changes nothing, and your personal overlay files are never written by it. [`adopt/ADOPT.md`](adopt/ADOPT.md) counts both installers as part of the attach.
